...
function init() {
  ...
  RDO = "[";
  RDC =
    "]"; /* opening and closing delimiters that embrace ratings of players in viewer display */
  DROPBAR = new Array(
    "br",
    "bq",
    "bb",
...
function generateLoadHtml(viewer) {
  ...
  tmp +=
    '<p align="center"><font color="#C0C0C0" size="6">Load Game</font></p>';
  tmp +=
    '<p>Here you may load a bughouse(holandany)/crazyhouse/chess game into the viewer. Also you may setup initial position! Press "Load" button when you are done. You may leave blank any field</p>';
  tmp +=
  ...
  tmp +=
    "<p>If you want to setup position different from the initial, paste it (or type in) below. Position must be in bfen format.</p>";
  ...
  tmp +=
    ...
    viewer +
    "'" +
    ',document.loadform.bpgn.value,document.loadform.bpgnord.value,document.loadform.bfena.value,document.loadform.bfenb.value);">';
...
function squareTextToIndex(sqtext) {
  ...
  var sqid =
    sqtext.toLowerCase().charCodeAt(0) -
    97 +
    8 * (8 - (sqtext.toLowerCase().charCodeAt(1) - 48));
...
function legalMove(piece, whomove, src, dest, mode) {
  ...
  if (piece == "p") {
    ...
    if (
      ...
      return true; //noncapture 2 sq's ahead
    if (
      Math.abs(dfile - sfile) == 1 &&
      srank == drank - dir &&
      (this.pos[dest] != "" || this.enpassantvalid(dest))
    )
      return true;
  ...
  if (piece == "n" || piece == "j") {
    piece = "n";
    if (
      Math.abs(dfile - sfile) + Math.abs(drank - srank) == 3 &&
      Math.abs(drank - srank) > 0 &&
      Math.abs(dfile - sfile) > 0
    )
      return true;
    return false;
  }
  ...
  if (piece == "q" || piece == "d") {
    piece = "q";
    return (
      this.legalmove("b", whomove, src, dest, 1) ||
      this.legalmove("r", whomove, src, dest, 1)
    );
  }
...
function legalMoveChecking(piece, whomove, src, dest, mode) {
  ...
  if (piece == "p") {
    ...
    if (
      Math.abs(dfile - sfile) == 1 &&
      srank == drank - dir //&&((this.pos[dest]!='')|| (this.enpassantvalid(dest)))
    )
      return true;
    /* capture. We don't check if we capture our own piece or opponents */
    return false;
...
function findMove(piece, whomove, spec, dest) {
  ...
  var cfile = "abcdefgh";
  var crank =
    "87654321"; /* strange order because index 0 corresposnds to a8 in array this.pos */
  var rmin = 0;
  var rmax = 8;
  var fmin = 0;
  var fmax = 8;
...
function decryptMove(bd, move) {
  ...
  if (move.charAt(1) == "@") {
    ...
    if (piece == "p" && (dest.charAt(1) == 8 || dest.charAt(1) == 1))
      return false;
...
function preload(path) {
  var gifs = new Array();
  for (var i = 0; i < 16; i++) gifs[i] = new Image();
...
function inDelim(
  ...
  del1,
  del2,
) /* returns a piece of a string str that sits between first occurences of delimiters del1 and del2 after position pos in str */ {
...
function strInsert(
  ...
  ind1,
  str2,
) /* inserts string str2 into string str1 at index ind1 */ {
...
function generateRtfTextPosition(fiWhiteUp) {
  ...
  lsHoldBlack = (lsHoldBlack + ",,,,,,,,,,,,,,,,").substring(
    0,
    Math.max(lsHoldBlack.length, 16),
  );
  lsHoldWhite = (lsHoldWhite + ",,,,,,,,,,,,,,,,").substring(
    0,
    Math.max(lsHoldWhite.length, 16),
  );
...
function refreshHighlight() {
  ...
  for (const boardId of "ab".split("")) {
    ...
    while (
      (boardId == "a" && currentNode.dmove.board == "b") ||
  ...
  while (ni < this.ngamesize) {
    ...
    while (ni < this.ngamesize) {
      ...
      if (this.nstate == BPGN_NORMAL) {
        ...
        if (x == "0") {
          if (ni + 2 <= this.ngamesize) {
            if (
              this.cgame.charAt(ni + 1) == "-" &&
              this.cgame.charAt(ni + 2) == "1"
            ) {
              this.ctoken = "0-1";
              ni = ni + 2;
              break;
      ...
      if (this.nstate == BPGN_SYMBOL) {
        if (
          this.cgame.charAt(ni) == " " ||
          this.cgame.charAt(ni) == TAB_ASCII_NUM ||
          this.cgame.charAt(ni) == CR_ASCII_NUM ||
          this.cgame.charAt(ni) == LF_ASCII_NUM
        )
          break;
        if (x.match(SYMB)) {
          cttt = x;
    ...
    if (
      ...
      this.nstate = BPGN_NORMAL;
      this.ctoken = ctmp;
    } else if (this.nstate == BPGN_COMMENT2) this.ctoken = ctmp;
...
function bpgnGetMove(nmovea, nmoveb, aside) {
  ...
  if (this.nlaststate != BPGN_INTEGER) {
    //we assume that it is move on board 'A'
  ...
  if (!this.gettoken()) {
    // missing board symbol (i.e. must be 'A', 'a', 'B', or 'b')
    return false;
  }
  if (this.nlaststate != BPGN_SYMBOL) {
    // invalid board symbol (i.e. must be 'A', 'a', 'B', or 'b')
  ...
  if (!bboarda1 && !bboarda2 && !bboardb1 && !bboardb2) {
    // invalid board symbol (i.e. must be 'A', 'a', 'B', or 'b')
...
function bpgnGetTime() {
  ...
  if (this.ctoken != "}") {
    // expecting '}' but we got something else
    this.BPGN_InsertQueue(this.nlaststate, this.ctoken);
...
function bpgnGetResult() {
  ...
  if (this.nlaststate != BPGN_NORMAL) {
    // expecting game result
    return false;
  }
  if (
    this.ctoken != BPGN_WHITE_WINS &&
    this.ctoken != BPGN_BLACK_WINS &&
    this.ctoken != BPGN_DRAW &&
    this.ctoken != BPGN_UNKNOWN
  ) {
    // invalid game result
    return false;
  }
...
function bpgnGetFirstNote() {
  ...
  if (this.gettoken()) {
    ...
    } // no, there is no note before the game starts so lets put the token back
    else {
      this.BPGN_InsertQueue(this.nlaststate, this.ctoken);
    }
  }
  ...
  while (true) {
    ...
    if (cMove == "(") {
      ...
      if (cLastBoard == BPGN_BOARD_A2) {
        nMA = nMoveA - 1;
        nMB = nMoveB;
      } else if (cLastBoard == BPGN_BOARD_B2) {
        nMA = nMoveA;
        nMB = nMoveB - 1;
      } else {
        nMA = nMoveA;
    ...
    if (this.gettoken()) {
      if (this.nlaststate == BPGN_COMMENT2) {
        ...
        if (this.gettoken()) {
          ...
          } else if (this.nlaststate == BPGN_NORMAL && this.ctoken == "(") {
            // get new node index
            nNewIndex = this.BPGN_GetNewNode();
            this.stuffnode(
              nNewIndex,
...
function bpgnSaveMove(nNode) {
  ...
  nT = cTmp.length;
  if (
    this.cSaveGameString.charAt(nL - 1) == " " ||
    this.cSaveGameString.charAt(nL - 1) == "("
  ) {
    if (nL + nT > BPGN_LINE_SIZE) {
      this.sgame += this.cSaveGameString + "\n";
...
function bpgnSaveGame() {
  ...
  this.sgame +=
    '[WhiteA "' + this.whitea + '"][WhiteAElo "' + this.whiteaelo + '"]\n';
  this.sgame +=
    '[BlackA "' + this.blacka + '"][BlackAElo "' + this.blackaelo + '"]\n';
  this.sgame +=
    '[WhiteB "' + this.whiteb + '"][WhiteBElo "' + this.whitebelo + '"]\n';
  this.sgame +=
    '[BlackB "' + this.blackb + '"][BlackBElo "' + this.blackbelo + '"]\n';
  ...
  this.sgame +=
    '[FEN "' + this.a.initial_bfen + " | " + this.b.initial_bfen + '"]\n';
  this.sgame += "\n";
...
function analysisMUp2(ind, viewer, bd) {
  ...
  var lsqorig = v.movestart.substring(
    v.movestart.length - 2,
    v.movestart.length,
  );
  var lsqtarget =
    "" + //+bd
    ((bd == "a" && v.a.flip == 0) || (bd == "b" && v.b.flip == 0)
      ? squareIndexToText(ind)
      : squareIndexToText(63 - ind));
  //alert(lsqorig+' vs '+lsqtarget);
  if (lsqorig == lsqtarget || ind > 63) return false;
  if (v.movestart == "") {
    v.mdown(ind, bd);
  } else {
    v.mup(ind, bd);
...
function bpgnRepairMove(fMove) {
  ...
  if (moveOnly.charAt(0) == "p") {
    //moves like p@ to P@
    moveOnly = moveOnly.replace("p", "P");
  } else if (moveOnly.charAt(0) <= "h" && moveOnly.charAt(0) >= "a") {
    if (moveOnly.charAt(2) == "x") {
      //only pawn takes like d4xe5
      moveOnly = moveOnly.charAt(0) + moveOnly.substr(2);
    } //only pawn move like d2d4
    else moveOnly = moveOnly.substr(2);
  }
...
function mDown(ind, bd) {
  ...
  if (ind < 64) {
    ...
    out.value =
      tbd.turn + "." + this.movestart.substr(1, this.movestart.length);
    return;
  }
  ind = ind - 64;
  if (tbd.flip != 0) {
...
function getBpgnHeaders(bpgntext) {
  ...
  this.gametime = parseInt(
    tmp[0],
  ); /* numerical: seconds per game, maybe redundant */
...
function drawBoard() /* generates html for the board */ {
  ...
  var tago = "<b>";
  var tagc =
    "</b>"; /*opening and closing tags to emphasize players handle and ratings (and times) */
  var up;
  var u;
  var d;
  var dn;
  ...
  if (this.reloadmode == "noreload" && this.displaymode == "diagram") {
    ...
    tmp =
      tmp +
      ' <input class="normal" type="text" size="12" name="' +
      "upclock" +
      bd +
      '" onFocus="this.blur();"> ';
  }
  ...
  if (this.reloadmode == "noreload") {
    ...
    tmp =
      tmp +
      ' <input type="text" size="16" name="up' +
      bd +
      '" onFocus="this.blur();"> ';
  }
  tmp = tmp + "</td>";
  ...
  for (i = 0; i < 8; i++) {
...

    {
      ...
      t +=
        ...
        bd +
        String(i + 1) +
        '" style="padding: 1px 2px 0 0; text-align: center; font-size:12px;">' +
        String(!flip ? 7 - i + 1 : i + 1) +
        "</td>";
    }
    for (j = 0; j < 8; j++) {
      ind = 8 * i + j;
      ...
      if (this.filebg) {
        t +=
          ...
          ")," +
          "url(" +
          ((i + j) % 2 ? this.gifs["b"] : this.gifs["w"]) +
          "); background-size: 100% 100%; background-repeat:no-repeat;";
      } else {
        t +=
          "background-color:" + ((i + j) % 2 ? this.gifs["b"] : this.gifs["w"]);
      }
      t += '">';
      t +=
        ...
        bd +
        "');" +
        '">'; //works click click and also drag move on pc and android where no mousedown
...

  {
    ...
    t +=
      '<tr id="coords2' +
      bd +
      '" style="text-align: center; font-size:12px"><td></td>';
    for (i = 1; i <= 8; i++) {
      t +=
        '<td style="padding: 2px 0 0 0"; id="coordinates2' +
        bd +
        i +
        '">' +
        String.fromCharCode(!flip ? i + 96 : 105 - i) +
        "</td>";
    }
    t += "</tr>";
  }
  ...
  for (i = 0; i < 5; i++) {
    for (j = 0; j < 2; j++) {
      ...
      t +=
        ...
        bd +
        "');" +
        '">'; //works click click and also drag move on pc and also android without mousedown
  ...
  tmp += t;
  tmp +=
    "<textarea style='display: none;' readonly disabled id='dntells" +
    bd +
    '\' rows=2 cols=10 value="bubu"></textarea>';
...
function bRefreshForm() {
  ...
  s = eval(
    "document." + this.viewername + "upmv" + bd,
  ); /* show whose move is now */
  ...
  if (this.displaymode != "diagram" || this.reloadmode != "noreload") {
    s = eval(
      "document." + this.viewername + ".upclock" + bd,
    ); /*refresh clocks */
  ...
  if (this.reloadmode != "noreload") {
    /*refresh handles */ s = eval("document." + this.viewername + ".up" + bd);
...
function execMove(bd, text) {
  ...
  isThereSuchAMoveAlready: {
    ...
    let isSameMove = (a, b) => {
      return (
        a.board == b.board &&
        a.fromsquare == b.fromsquare &&
        a.tosquare == b.tosquare &&
        a.frompiece == b.frompiece
      );
    };
...
<input type="button" value="Set Layout"
  ...
  lGifNumber = parseInt(
    lobj.substring(
      lobj.indexOf("gif") + 3,
      lobj.indexOf("/", lobj.indexOf("gif") + 3),
    ),
  );
...
function drawControl(color, mode) {
  ...
  var t =
    '<center style="display: none;"><table border="0" cellpadding="0" cellspacing="0" width="' +
    vd +
    '" bgcolor="' +
    color +
    '">';
  ...
  t += "<tr>";
  t +=
    '<td align="left" valign="top" style="font: 13px arial narrow, sans-serif;">';
  t += '<p style="font: 10px"></p>';

  //t+='<p style="font: 8px"></p>';
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL1" type="button" value="' +
    BTELL1 +
    '" onclick="interfacePtell(\'' +
    BTELL1 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL3" type="button" value="' +
    BTELL3 +
    '" onclick="interfacePtell(\'' +
    BTELL3 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL5" type="button" value="' +
    BTELL5 +
    '" onclick="interfacePtell(\'' +
    BTELL5 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL7" type="button" value="' +
    BTELL7 +
    '" onclick="interfacePtell(\'' +
    BTELL7 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL9" type="button" value="' +
    BTELL9 +
    '" onclick="interfacePtell(\'' +
    BTELL9 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL11" type="button" value="' +
    BTELL11 +
    '" onclick="interfacePtell(\'' +
    BTELL11 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL13" type="button" value="' +
    BTELL13 +
    '" onclick="interfacePtell(\'' +
    BTELL13 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL15" type="button" value="' +
    BTELL15 +
    '" onclick="interfacePtell(\'' +
    BTELL15 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL17" type="button" value="' +
    BTELL17 +
    '" onclick="interfacePtell(\'' +
    BTELL17 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL19" type="button" value="' +
    BTELL19 +
    '" onclick="interfacePtell(\'' +
    BTELL19 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL21" type="button" value="' +
    BTELL21 +
    '" onclick="interfacePtell(\'' +
    BTELL21 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL23" type="button" value="' +
    BTELL23 +
    '" onclick="interfacePtell(\'' +
    BTELL23 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL25" type="button" value="' +
    BTELL25 +
    '" onclick="interfacePtell(\'' +
    BTELL25 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL27" type="button" value="' +
    BTELL27 +
    '" onclick="interfacePtell(\'' +
    BTELL27 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL29" type="button" value="' +
    BTELL29 +
    '" onclick="interfacePtell(\'' +
    BTELL29 +
    "')\">";
  ...
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL2" type="button" value="' +
    BTELL2 +
    '" onclick="interfacePtell(\'' +
    BTELL2 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL4" type="button" value="' +
    BTELL4 +
    '" onclick="interfacePtell(\'' +
    BTELL4 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL6" type="button" value="' +
    BTELL6 +
    '" onclick="interfacePtell(\'' +
    BTELL6 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL8" type="button" value="' +
    BTELL8 +
    '" onclick="interfacePtell(\'' +
    BTELL8 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL10" type="button" value="' +
    BTELL10 +
    '" onclick="interfacePtell(\'' +
    BTELL10 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL12" type="button" value="' +
    BTELL12 +
    '" onclick="interfacePtell(\'' +
    BTELL12 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL14" type="button" value="' +
    BTELL14 +
    '" onclick="interfacePtell(\'' +
    BTELL14 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL16" type="button" value="' +
    BTELL16 +
    '" onclick="interfacePtell(\'' +
    BTELL16 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL18" type="button" value="' +
    BTELL18 +
    '" onclick="interfacePtell(\'' +
    BTELL18 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL20" type="button" value="' +
    BTELL20 +
    '" onclick="interfacePtell(\'' +
    BTELL20 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL22" type="button" value="' +
    BTELL22 +
    '" onclick="interfacePtell(\'' +
    BTELL22 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL24" type="button" value="' +
    BTELL24 +
    '" onclick="interfacePtell(\'' +
    BTELL24 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL26" type="button" value="' +
    BTELL26 +
    '" onclick="interfacePtell(\'' +
    BTELL26 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL28" type="button" value="' +
    BTELL28 +
    '" onclick="interfacePtell(\'' +
    BTELL28 +
    "')\">";
  t +=
    '<input style="width:64px; height:40px;"  id="BTELL30" type="button" value="' +
    BTELL30 +
    '" onclick="interfacePtell(\'' +
    BTELL30 +
    "')\">";
  ...
  t +=
    '<center><table border="0" cellpadding="0" cellspacing="0" width="' +
    vd +
    '" bgcolor="' +
    color +
    '">';
  ...
  if (this.numboard != 1) {
    ...
    t +=
      " 'https://online-dropchess-stockfish.onrender.com/analysis.html?fen=' + bfen_to_bracket_notation( window.v1.a.generatebfen() ";
    t += "),'_blank'" + ')">';
  ...
  if (this.numboard != 1) {
    ...
    t += '<input type="button" value="analyse" onclick="window.open(';
    t +=
      " 'https://online-dropchess-stockfish.onrender.com/analysis.html?fen=' + bfen_to_bracket_notation( window.v1.b.generatebfen() ";
    t += "),'_blank'" + ')">';
...
function drawInfo(color) {
  ...
  t =
    t +
    '<center><table border="0" cellpadding="0" cellspacing="0" bgcolor="' +
    color +
    '">';
  t = t + "<tr>";
  ...
  t +=
    ...
    ",document." +
    this.viewername +
    '.nextmove.selectedIndex);" >';
  for (i = 0; i < MAX_NEXT; i++) opt = opt + "<option>";
  t = t + "<option>" + ph + opt;
  t = t + "</select></td>";
  t =
    t +
    '<td rowspan="2" valign="top"><textarea onchange="saveNote(' +
    "'" +
    this.viewername +
    "'" +
    ')" name="comment"' +
...
function drawViewer(color) {
  ...
  tmp += "<tr>";
  tmp +=
    '<td><table border="0" cellpadding="2" bgcolor="' +
    color +
    '">'; /* head of board's table */
  tmp += "<tr>";
  tmp +=
    '<td  width="' +
    this.vwidth +
  ...
  if (this.displaymode != "diagram") {
    tmp +=
      "<tr><td>" +
      this.drawcontrol(color, this.displaymode) +
      "</td></tr>"; /* drawing controls */
...
function game(
  ...
  this.viewername =
    the_viewer_name; /* prefix of the names of all the elements (forms, images, etc.) of a viewer on which the game is displayed. E.g.  if names of the forms are ...viewwhitea, viewwhiteb,... then viewername=="view" */
  ...
  this.displaymode =
    display_mode.toLowerCase(); /* Defines appearance of the viewer. displaymode=="Diagram" <=>Only diagram with position, no control buttons, no information window (info+ next move + comments); displaymode=="Playback" <=> no information window, but controls are present. By default full configuration, i.e. both controls and information are present */

  this.reloadmode =
    reload_mode.toLowerCase(); /* Determines whether this viewer will be able to show different games. reloadmode=="NoReload" <=> handles+ratings will be displayed via html rather than input form and thus would look better, but be unchangeable */

  this.capturemode =
    capture_mode.toLowerCase(); /* 3 possibilities to handle captures: bughouse(holandany), crazyhouse, and chess(i.e. captured pieces vanish), 'bug', 'zh' and 'chess' respectively if none is selected we the mode is choosed based on bpgn loaded */
  if (
    this.capturemode != "bug" &&
    this.capturemode != "zh" &&
  ...
  this.gifs = initPieceGifs(
    ...
    this.whitesq,
    this.blacksq,
  ); /* associative array with names of gif files used to draw pieces and board*/
  //alert('this.gifs[wbd]:'+this.gifs["wbd"]+",");

  this.flip =
    flip == "flip"
      ? 1
      : 0; /* this.flip == 0 <=> no flip, this.flip==1 <=> flipped display */
  this.movestart = "";
